    ...
    request({
        ...
        message.channel.send("```" + translated + "```").then(mess => {
            if(mess.channel.type != "dm")
                mess.delete({timeout: autoDelDelay})
        });
        message.delete({timeout: autoDelDelay}).catch(err => {});
    });
}
...
function googleTranslate(message, args){
    ...
    translate(text[0], {from: firstLang, to: secondLang}).then(res => {
        ...
        message.delete({timeout: autoDelDelay}).catch(err => {console.log(err)});
    });
}
function sendLangs(message){
        ...
        message.delete({timeout: settings.autoDelDelay}).catch(err => {console.log(err)});
        return;
}
...
function imdb(cmd, args, channel){
    ...
    request({
        ...
        if(data.Response == "False"){
            channel.send("```" + data.Error + "```").then(sent => sent.delete({timeout: autoDelDelay}));
            return
        }
...
function words(type, count, args, channel){
    request({
        ...
        if(err) 
            return console.error(err);
        if(data.length == 0){
            channel.send("```not found```").then(sent => sent.delete({timeout: autoDelDelay}));
            return
        }
        let list = "";
...
function vocabulary(args, channel, author){
    owlClient.define(args).then(function(result){
        ...
        channel.send("```" + definition + "```").then(sent => {
			...
			let collector = sent.createReactionCollector(settings.filter, { time: 100000 });
