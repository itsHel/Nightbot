...
const officialPack = JSON.parse(fs.readFileSync("data/cah/official.json", "utf8", function(){}));
const unofficialPack = JSON.parse(fs.readFileSync("data/cah/unofficial.json", "utf8", function(){}));
...
function cardsAgainstHumanity(channel, modroles){
                    ...
                    case "packs":
                        ...
                        switch(args[2]){
                            ...
                            case "unofficial":
                                ...
                                break;
                            case "both":
                                    this.settings.packs = [officialPack, unofficialPack];
                                break;
                            default:
                                channel.send("```Wrong argument```");
                            }
    ...
    this.generateBlackcard = function(){
        let pack = this.settings.packs[Math.trunc(Math.random() * this.settings.packs.length)];
        let blackcard;

        do {
            blackcard = pack.blackCards[Math.trunc(Math.random() * pack.blackCards.length)];
        } while(historyBlackcards.filter(card => card == blackcard).length || (!this.settings.triplesOn && blackcard.pick == 3));
    ...
    this.waitForBlank = async function(discordPlayer){
        ...
        await discordPlayer.createDM().then(async channel => {
            await channel
                .awaitMessages(messFilter, {max: 1, time: this.settings.answerDelay - 20000})
                    .then(collected => {
    ...
    this.allPlayersPicked = function(){
        ...
        for(let i = 0; i < this.players.length; i++){
            ...
            (devMode || logMode) && mine.log("collector stopped for: " + this.players[i].nick + "", undefined, mine.getLine(), mine.getFunc());   
        }
        ...
        channel.send(embed)
            ...
            // Czar picking message
                ...
                let collector = m.createReactionCollector(czarFilter, { time: this.settings.answerDelay + 20000, max: 1});
                ...
                collector.on("end", collected => {
                    ...
                    if(collected.size == 0){
                        (devMode || logMode) && console.log("Czar collected size is 0")
    ...
    this.sendQuestion = function(player){
        ...
        player.discordPlayer.send(playerEmbed)
            ...
            // Question sent to player
                ...
                let answers = 0;
                let blank = "";
                player.collector = m.createReactionCollector(filter, { time: this.settings.answerDelay, dispose: true });
                ...
                player.collector.on('collect', async(r) => {                                     // Waiting for players picks
                    ...
                    for(let o = 0; o < player.cards.length; o++){
                        if(r.emoji.name == emojis[o] || player.cards.length == o + 1){
                            ...
                            (devMode || logMode) && console.log("Active cards: " + player.activeCards);
