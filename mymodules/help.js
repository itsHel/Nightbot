...
function help(channel) {
    let embed = new discord.MessageEmbed()
        ...
        .addField(
            settings.prefix + "remind *message* ;time",
            "Bot will DM you your message after time passes, m - mins, h - hours, d - days OR 16:16 format\n\nexample: '*" +
                settings.prefix +
                "remind regret life choices ;1h 10m*'",
        )
        .addField(
            settings.prefix + "duel user",
            "Will start rock-paper-scissors with chosen user",
        )
        .addField(
            settings.prefix + "random *X*",
            "Picks random *X*, default is 2",
        )
        .addField(settings.prefix + "toss", "Head or Tails")
        .addField(
            settings.prefix + "movie",
            "Movie info, can can specify year after ';', example: '*" +
                settings.prefix +
                "movie freaks ;2018*'",
        )
        ...
        .addField(
            settings.prefix + "setban *X*",
            "Sets autoban after user leaves **X** is number of days, 24 days is maximum",
        )
        ...
        .addField(
            settings.prefix + "defaultrole",
            "Default role added to new server member, example: " +
                settings.prefix +
                "defaultrole peasant",
        )
        .addField(
            settings.prefix + "def, syn, sug, rhyme",
            "Definition of a word\nsynonyms\nsuggestions\nrhymes with word",
        )
        .addField(
            settings.prefix + "tr *message* ;from into",
            "Translate, example: '*" +
                settings.prefix +
                "tr hello ;english spanish*'\nuse '" +
                settings.prefix +
                "trlist' for list of supported languages",
        )
        .addField(
            settings.prefix + "c/confess *message*",
            "(can use Bots DM) Sends your message annonymously into confession room",
        )
        .addField(
            settings.prefix + "announce *message*",
            "(can use Bots DM) Sends your message annonymously into announce room",
        )
        .addField(
            settings.prefix + "dm user *message*",
            '(can use Bots DM) Sends your message annonymously into chosen user DMs, use their main nickname and double quotes for names with spaces - "hell bot"',
        )
        .addField(
            settings.prefix + "gif(a) tags",
            "Gif, a means anime, example: '*" +
                settings.prefix +
                "gifa kiss winter*'",
        )
        .addField(
            settings.prefix + "poll text",
            "Creates poll in one of rooms, annonymous by default (can be used in dm), add ';' sign to show your nickname with poll",
        )
        .addField(
            settings.prefix + "cah",
            "Shows guide to Cards Against Humanity\n" +
                settings.prefix +
                "cah set/settings shows avaible settings",
        )
        .addField(
            settings.prefix + "rr",
            "Play Russian Roulette with other user(s), " +
                settings.prefix +
                "rr shows more\n\nexample: '*" +
                settings.prefix +
                "rr @user*'",
        ...
        .addField(
            settings.prefix + "butt, " + settings.prefix + "boobs",
            ":flushed: pic",
        )
        .addField(
            settings.prefix + "p(f) tags",
            "Nsfw pic, f means gif, example: '*" +
                settings.prefix +
                "pf double_penetration*'",
        )
        .addField(
            settings.prefix + "hentai *tag*",
            "Hentai pic, '" +
                settings.prefix +
                "hentai tags' shows avaible tags, example: '*" +
                settings.prefix +
                "hentai ass*'\n**Avaible tags:** ass, bdsm, cum, femdom, doujin, maid, orgy, panties, wallpaper, pussy, succubus",
        )
        .addField(
            "\nRooms to setup:",
            "generalRoom, pinRoom, pollRoom, confessRoom, delRoom, welcomeRoom, announceRoom\n\nSetup: " +
                settings.prefix +
                "addroom/editroom pinRoom #gallery",
        )
        .setColor("#d652a6")
        .setTitle("Help");
    ...
    switch (cmd) {
        case "reddit":
            embed = new discord.MessageEmbed()
                ...
                .addField(
                    "\u2800",
                    "Automatically sends feed from selected subreddits into designated rooms (refreshes every 5 hours)\nYou must set up *redditTheatre* or *redditText* or *redditNsfw* rooms first\nVideos and some gifs cant show in discord directly but only as links, therefore only option to turn them on\n**Example:** " +
                        settings.prefix +
                        "addroom reddittheatre #myreddit\n\n**redditTheatre** - Observes image feed\n**redditText** - Observes text feed\n\u2800",
                )
                .addField(
                    settings.prefix + "addreddit",
                    "Adds/edits new subreddit observer for text or image posts\n**Format:** " +
                        settings.prefix +
                        "addreddit subreddit min-upvotes text/img/nsfw ignoreVideos\n**Example:** " +
                        settings.prefix +
                        "addreddit philosophy 1000 img true",
                )
                .addField(
                    settings.prefix + "removereddit",
                    "Removes subreddit observer\n**Example:** " +
                        settings.prefix +
                        "removereddit news",
                )
                .addField(
                    settings.prefix + "reddits",
        ...
        case "room":
            embed = new discord.MessageEmbed()
                .setTitle("Rooms")
                .addField(
                    settings.prefix + "rooms",
                    "Shows list of active rooms",
                )
                .addField(
                    settings.prefix + "addroom",
                    "Adds/edits new room\n\n**Example:** " +
                        settings.prefix +
                        "addroom confessRoom #myconfessions",
                )
                .addField(
                    settings.prefix + "removeroom",
                    "Removes room\n**Example:** " +
                        settings.prefix +
                        "removeroom confessRoom",
                )
                .addField(
                    "Rooms to setup: ",
        ...
        case "pins":
            embed = new discord.MessageEmbed()
                ...
                .addField(
                    "You must setup room first\n\u2800\n",
                    "Example: **" +
                        settings.prefix +
                        "addroom pinRoom *#mypins***\n\n",
                )
                .addField(
                    settings.prefix + "nopins",
                    "Adds room to no pins list\nExample: **" +
                        settings.prefix +
                        "nopins *#mainroom***",
                )
                .addField(
                    settings.prefix + "nopinslist",
                    "Lists all no-pins rooms",
                )
                .addField(
                    settings.prefix + "pinstoggle",
        ...
        case "remind":
            embed = new discord.MessageEmbed()
                ...
                .setColor("#d652a6")
                .setDescription(
                    "Sends you message after timeout\n\nExample: **" +
                        settings.prefix +
                        "remind drink water ;1h 10m**\nExample: **" +
                        settings.prefix +
        ...
        case "kickrole":
            embed = new discord.MessageEmbed()
                ...
                .setColor("#d652a6")
                .setDescription(
                    "Kicks users who are having **only** this role, if its 'everyone' it kicks users without role\n\nExample:\n**" +
                        settings.prefix +
                        "kickrole peasant**",
                );
            channel.send({ embeds: [embed] });
        ...
        case "words":
            embed = new discord.MessageEmbed()
                .setTitle("Vocabulary")
                .addField(
                    settings.prefix + "def *word*",
                    "Shows definition of word",
                )
                .addField(
                    settings.prefix + "syn *word*",
                    "Shows synonyms of word",
                )
                .addField(
                    settings.prefix + "rhyme *word*",
                    "Shows rhyme of word",
                )
                .addField(
                    settings.prefix + "sug *word*",
        ...
        case "russianroulette":
            embed = new discord.MessageEmbed()
                ...
                .setColor("#d652a6")
                .setDescription(
                    "Challenges player(s) to Russian Roulette\n\nExample: **" +
                        settings.prefix +
                        "rr *@player* 1 kick**\nFormat: **" +
                        settings.prefix +
