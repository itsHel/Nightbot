...
function listFunctions(cmd, args, message, guildSettings){
    ...
    if(temp[0] == "create" || temp[0] == "c"){
        ...
        channel.send("```list " + list + " created```").then(sent => sent.delete({timeout: settings.autoDelDelay}));
        lists.push(list);
    } else if(temp[0] == "add"){
        //ADDING
        fs.appendFileSync("data/lists/" + list + ".txt", args.substring(5 + temp[1]  .length) + "\n", function(){});
        channel.send("```" + args.substring(5 + temp[1].length) + " added to the " + list + " list```").then(sent => sent.delete({timeout: settings.autoDelDelay}));
    } else if(temp[0] == "del"){
        //DELETING items
        ...
        channel.send("```number " + temp[2] + " removed from " + list + "```").then(sent => sent.delete({timeout: settings.autoDelDelay}));
        ...
        fs.unlink("data/lists/" + list + ".txt", err => {
            if(err){
                console.log(err);
                channel.send("```not a list```").then(sent => sent.delete({timeout: settings.autoDelDelay}));
                return;
            }
        });
        channel.send("```deleted```").then(sent => sent.delete({timeout: settings.autoDelDelay}));
    } else {
