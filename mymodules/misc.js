...
async function autoPin(channel, pinChannel, guildId){
    ...
    }).catch(err => { console.log(err); log("autoPin error:\n" + err.message); });
...
function russianRoulette(args, message, modroles = []){
    ...
    let allMentionedPlayers = [], activePlayers = [];
    ...
    function start(){
        setTimeout(function(){
            // Randomize array
            activePlayers = activePlayers.map(value => ({ value, sort: Math.random() })).sort((a, b) => a.sort - b.sort).map(({ value }) => value);
...
function sendToChannel(text, targetChannel, channel){
    ...
    channel.send("sent").then(sent => sent.delete({timeout: settings.autoDelDelay}));
}
        ...
        if(!found){
            ...
            setTimeout(function(){
                if(Math.floor(Math.random() * 50) == 0){
                    channel.send("YOU'LL PAY FOR YOUR CRIMES AGAINST HUMANITY!!!").then(sent => sent.delete({timeout: 2000}));
                    return;
                }  
...
function countDown(i, delay, channel){
    setTimeout(function(){
        if(!i){
            channel.send("Now").then(sent => sent.delete({timeout: settings.autoDelDelay * 10}));
            return;
        }
        channel.send(i--).then(sent => sent.delete({timeout: settings.autoDelDelay * 10}));
...
function emotesShow(stats, message, client){
    ...
    emotesArr.sort(function(a, b){return b[1] - a[1]});
...
function isAdmin(message, modRoles = [], response = true){
    ...
    if(member.roles.cache.find(role => {
        ...
        if(response)
            message.channel.send(noModMessage).then(mess => mess.delete({timeout: settings.autoDelDelay}));
...
function sendDm(args, message, client){                 // Mess?
    if(!args){
        message.channel.send("```Message empty```").then(sent => sent.delete({timeout: settings.autoDelDelay}));
        return;
    }
    let nick;
    let text = args;
    ...
    client.users.fetch(nick.replace("<@","").replace(">","").replace("!","")).then((user) => {
        ...
        message.channel.send("```Sent```").then(sent => sent.delete({timeout: settings.autoDelDelay}));
    }).catch((err) => {
        nick = nick.replace(/^@/, "");
...
function saveImages(message){
    message.channel.messages.fetch({limit: parseInt(args)}).then(messages => {
