...
function privateDmMessage(user) {
    return (
        "Cannot send DM to " +
        user +
        ', change your "Privacy & safety" settings please'
    );
***REMOVED***
            ...
            messages.forEach((mess) => {
                if (mess.pinned) {
                    ...
                    if (mess.embeds.length) {
                        img =
                            mess.embeds[0].image == null
                                ? ""
                                : mess.embeds[0].image.url;
                        vid =
                            mess.embeds[0].video == null
                                ? ""
                                : mess.embeds[0].video.url;
                        link =
                            mess.embeds[0].url == null
                                ? ""
                                : mess.embeds[0].url;
                        title =
                            mess.embeds[0].title == null
                                ? ""
                                : mess.embeds[0].title;
                        if (img == "")
                            img =
                                mess.embeds[0].thumbnail == null
                                    ? ""
                                    : mess.embeds[0].thumbnail.url;
...
***REMOVED***
    ...
    allMentionedPlayers = allMentionedPlayers.filter(
        (x, i, a) => a.indexOf(x) == i && x != message.member,
    );
    ...
    ***REMOVED***
    ...
    if (type) {
        let alertText =
            "Loser(s) will be dishonorably **" +
            (type == "kick" ? "kicked" : "banned") +
            "** from server!";
    ...
    ***REMOVED*** else {
        ***REMOVED***
            return (
                allMentionedPlayers.filter((player) => player.user == user)
                    .length &&
    ...
    ***REMOVED***
        ...
        collector.on("collect", (reaction, user) => {
            if (reaction.emoji.name == emojisClassic.check) {
                let reactMember = allMentionedPlayers.find(
    ...
    function start() {
        setTimeout(function () {
            ...
            activePlayers = activePlayers
                .map((value) => ({ value, sort: Math.random() ***REMOVED***))
            ...
            setTimeout(function () {
                if (round != 1) {
                    for (let i = 0; i < step; i++) {
                        ...
                        do {
                            var newBullet =
                                Math.floor(Math.random() * drumSize) + 1;
                        ***REMOVED*** while (
                            bullets.filter((bullet) => bullet == newBullet)
                                .length
                        );
    ...
    if (args) {
        ...
        ***REMOVED***
        ...
        if (!found) {
            ...
            ***REMOVED*** else {
                channel.send(
                    "Thank you for watching <a:milkBow:" + emojis.milkBow + ">",
                );
            ***REMOVED***
...
function getRatingMessage(ratingsAll) {
    ...
    for (let i = 0; i < ratingsAll.length; i++) {
        msg +=
            ratingsAll[i].user.toString() + " - " + ratingsAll[i].rating + "\n";
    ***REMOVED***
    msg = msg.slice(0, -1);
...
***REMOVED***
    ...
    let text = emotesArr
        .map((emote) => {
            ...
            let emoteID = client.emojis.cache.find(
                (emoji) => emoji.name == emote[0],
            );
...
function isAdmin(message, modRoles = [], response = true) {
    if (
        (message.member && message.member.id == settings.Hel) ||
        (message.author && message.author.id == settings.Hel)
    )
        return true;
    ...
    if (
        member.roles.cache.find((role) => {
            for (let i = 0; i < modRoles.length; i++)
                if (role.name == modRoles[i]) return true;
        ***REMOVED***)
    ) {
        return true;
    ***REMOVED*** else {
        if (response) {
            message.channel
                .send(noModMessage)
                .then((mess) =>
                    setTimeout(() => mess.delete(), settings.autoDelDelay),
                );
        ***REMOVED***
    ...
    try {
        ...
        ***REMOVED*** else {
            ...
            for (const line of emotes) {
                ...
                let emoji = line.slice(
                    line.lastIndexOf(":") + 1,
                    line.indexOf(">"),
                );
                let emojiName = line.slice(
                    line.indexOf(":") + 1,
                    line.lastIndexOf(":"),
                );
                let emojiUrl;
                ...
                if (line[0] == "a") {
                    // Animated
                    emojiUrl =
                        "https://cdn.discordapp.com/emojis/" + emoji + ".gif";
                ***REMOVED*** else {
                    // Non-animated
                    emojiUrl =
    ...
    client.users
        ...
        ***REMOVED***)
        .catch((err) => {
            ...
            if (
                client.users.cache.find((user) => user.username == nick) == null
            ) {
            ...
            ***REMOVED***
            client.users
                .fetch(
                    client.users.cache.find((user) => user.username == nick).id,
                )
                .then((user) => {
                    user.send(text)
                        .catch((err) => {
        ...
        if (
            ...
            (roleCount == 2 &&
                member.roles.cache.some(
                    (memberRole) =>
                        memberRole.name.toLowerCase() == role.toLowerCase(),
                ))
        ) {
...
function saveImages(message) {
    message.channel.messages
        .fetch({ limit: parseInt(args) ***REMOVED***)
        .then((messages) => {
                    ...
                    request
                        ...
                        .pipe(
                            fs.createWriteStream(
                                path + Date.now() + pic.filename,
                            ),
                        ); // Write file to the system synchronously
            ***REMOVED***
        ***REMOVED***
