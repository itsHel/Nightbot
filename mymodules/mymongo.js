...
function updateSchema(values, schema, guildid, reset = false){
    ...
    switch(schema){
        case "room":
            ...
            newSchema.findOneAndUpdate({guildid: guildid}, values, {upsert: true})
                .then(result => {})
                .catch(err => console.log(err));
            break;
        case "reddit":
            newSchema = mongoose.model("reddit", redditSchema);
            newSchema.findOneAndUpdate({guildid: guildid, reddit: values.reddit, type: values.type}, values, {upsert: true})
                .then(result => {})
                .catch(err => console.log(err));
            break;
        case "settings":
            ...
            if(values.nopinsrooms || values.bans || values.modroles){
                ...
                if(!reset){
                    ...
                    values = {$addToSet: {[property]: values[Object.keys(values)[0]]}};
                }
            }
...
function addReminder(userid, message, time){
    ...
    let reminder = new newSchema({userid:userid, message: message, time: time});
    reminder.save().then(result => console.log(result));
}

async function getReminders(userid){
    let newSchema = mongoose.model("reminder", reminderSchema);
    await newSchema.deleteMany({time: {$lte: new Date().getTime()}})
        .then()
        .catch(err => console.log(err));
...
async function getRedditHistory(guildid, reddit, type){
    ...
    return await newSchema.find({guildid: guildid, reddit:reddit, type: type})
        .then((res) => {
...
async function getSetting(guildid, type){
    ...
    return await newSchema.find({guildid: guildid})
        .then((res) => {return res[0][type]});
}

async function getSettings(guilds){
    let newSchema = mongoose.model("settings", settingsSchema);

    return await newSchema.find({guildid: {$in: guilds}})
        .then((res) => {
            let settings = {};
            let i = 0;
...
function deleteRoleMessage(messageid){
    ...
    newSchema.deleteOne({messageid:messageid})
        .then()
        .catch(err => console.log(err));
}
...
module.exports = {
    ...
    // const blogSchema = new mongoose.Schema({title: {type: String}, title22: {type: String}});
    ...
    // Blog.findOneAndUpdate({title:"MYTITTLE-----------"}, {title22: "UPDATEEEEEEEED"}, {upsert: true})
    //     .then(result => console.log(result))
    //     .catch(err => console.log(err));


    // Blog.update({title:"MYTITTLE-----------"}, {title22: "U"})
    //     .then(result => console.log(result))
    //     .catch(err => console.log(err));

    // const blog = new Blog({title:"MYTITTLE-----------",title22:"newroom"});
    // blog.save();
