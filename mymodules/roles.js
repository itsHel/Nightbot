...
async function newRole(user, roleChannel){
    ...
    user.send("Enter title (enter . to skip)").then(() => {
        user.createDM().then(chan => {
            if(!roleChannel){
                chan.send("```Roleroom does not exist```");
                throw "no channel";
            }
            chan.awaitMessages(function(){return true;}, {max: 1, time: 60000}).then(async(collected) => {
                if(!collected.first())
                    throw "timeout";
                ...
                chan.awaitMessages(function(){return true;}, {max: 1, time: 60000}).then(async(collected) => {
                    if(!collected.first())
                        throw "timeout";
                    await chan.send("Enter emote - matching role, example: ':peace: - peace'\nMultiple entries must be split by newline (shift + enter)");
                    role.header = collected.first().content;

                    chan.awaitMessages(function(){return true;}, {max: 1, time: 60000}).then(async(collected) => {
                        if(!collected.first())
                            throw "timeout";
                        await chan.send("Enter footer (enter . to skip)");
                        role.roles = collected.first().content;
                        
                        chan.awaitMessages(function(){return true;}, {max: 1, time: 60000}).then(async(collected) => {
                            if(!collected.first())
                                throw "timeout";
                            await chan.send("Unique role? Y/N (user can have only one role from this category if you pick Yes)");
                            role.footer = collected.first().content;

                            chan.awaitMessages(function(){return true;}, {max: 1, time: 60000}).then(async(collected) => {
                                if(!collected.first())
                                    throw "timeout";
                                if(collected.first().content[0].toLowerCase() == "y")
                                    role.unique = true;
                                else
                                    role.unique = false;
                                
                                await chan.send("```Unique set to " + ((role.unique) ? "Yes" : "No") + "```");
                            
                                role.roles.split("\n").forEach(row => {
                                    emotes.push(row.slice(0, row.indexOf(" "))); 
                                    roles.push(row.slice(row.indexOf("- ") + 2, row.length));
                                });
                                
                                let embed = new discord.MessageEmbed().setColor("#ff6600").setDescription(role.roles);
                                if(role.title != ".")
                                    embed.setTitle(role.title);
                                if(role.header != ".")
                                ...
                                roleChannel.send(embed).then(async (mess) => {
                                    for(let i = 0; i < emotes.length; i++)
                                    {
                                        ...
                                        let emoji;
                                        if(id)
                                            emoji = mess.guild.emojis.resolveID(id[1]);
                                        else
                                            emoji = emotes[i];
                                        try{
                                            await mess.react(emoji).catch((err) => {
                                                error = err.message; 
                                                chan.send("```Error: " + err.message + ", with: " + emotes[i] + "```"); 
                                                console.log("error:");
                                                console.log(error);
                                            });
                                        } catch(err){}
                                    ...
                                    let collector = mess.createReactionCollector(filter, { time: 0, dispose: true });
                                    ...
                                    mongo.saveRoleMessage(embedMessage.embeds[0].description, embedMessage.id, embedMessage.channel.id, embedMessage.guild.id, role.unique);
                                });
                                chan.send(roleChannel.toString());
                            });
                        }).catch(err => {
                            chan.send("```Cancelled```");
                            console.log(err);
        ...
        for(let i = 0; i < roles[guild].length; i++){
            if(client.channels.cache.get(roles[guild][i].channelid)){
                client.channels.cache.get(roles[guild][i].channelid).messages.fetch(roles[guild][i].messageid)
                    .then(msg => {
                        let collector = msg.createReactionCollector(filter, { time: 0, dispose: true });
                    ...
                    if(unique){
                        roles.forEach(removeRole => {
                            if(member.roles.cache.some(memberRole => memberRole.name == removeRole))
                                try{
                                    let roleObject = collected.message.guild.roles.cache.find(guideRole => guideRole.name.toLowerCase() === removeRole.toLowerCase());
...
function setDefaultRole(guildSetting, role, message){
    if(role.length && message.guild.roles.cache.find(guildRole => guildRole.name.toLowerCase() === role.toLowerCase()) == undefined){
        message.channel.send("```Role not found```"); //.then(mess => mess.delete({timeout: settings.autoDelDelay}));
        return;
    }
    
    guildSetting.defaultrole = role;
    mongo.updateSchema({defaultrole: role}, "settings", message.guild.id);
    if(role.length == 0){
