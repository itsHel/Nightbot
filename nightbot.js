...
const client = new discord.Client({ intents: allIntents, partials: ["CHANNEL"] });
...
client.on("message", async message => {
    ...
    try{
        ...
        switch(cmd){
            case "stealemote": case "addemoji": case "stealemoji": case "addemote":
            ...
            case "countdown": case "cd":
                ...
                message.delete({timeout: settings.autoDelDelay * 10});
                break;
            case "quote": case "q":
            ...
            case "trlist":          // Supported translation languages list
                ...
                break;
            case "movie": case "movief": case "movies":
                if(args.length == 0){
            ...
            case "pinstoggle":   
                ...
                break;
            case "pin": case "pins": case "nopins":
                ...
                if(mine.isAdmin(message, guildSettings[guildId].modroles)){
                    ...
                    if(channel == undefined){
                        message.channel.send("```Room not found```").then(mess => mess.delete({timeout: settings.autoDelDelay}));
                        return;
                    }
                    ...
                    message.channel.send("```Added```").then(mess => mess.delete({timeout: settings.autoDelDelay}));
                }
                break;
            ...
            case "reddits": case "redditlist": case "redditslist":
                ...
                break;
            case "removeroom": case "deleteroom":
            ...
            case "addroom": case "editroom": case "setroom": case "changeroom": case "room": case "setup":
                ...
                break;
            case "roomlist": case "roomslist": case "rooms":
                if(mine.isAdmin(message, guildSettings[guildId].modroles)){
                    ...
                    let embed = new discord.MessageEmbed({fields: [name, channelObj]}).setFooter("\u2800".repeat(50));          // Embed sizing     \u2800 = empty space
                    message.channel.send(embed);
                }
                break;
            case "addmod": case "addmods": case "addmodrole":
                ...
                if(mine.isAdmin(message, guildSettings[guildId].modroles)){
					if(args.length){
						...
						if(!modRole){
							message.channel.send("```Role '" + newRole + "' not found```").then(mess => mess.delete({timeout: settings.autoDelDelay}));
							break;
						}
            ...
            case "help":
                ...
                return;
            // Lewds
            case "p": case "pf":                // f means Gif
                picapis.danbooru(cmd, args, message.channel);
                break;
            case "hentai": case "anime":        // anime means sfw
    ...
    } catch(err){commandError(message, err)}
});
...
client.on("guildMemberRemove", (member) => {
    try{
        ...
        if(guildSettings[member.guild.id].leavebandays != 0){
            member.ban("leaving server temporary ban: " + guildSettings[member.guild.id].leavebandays + " days")
            ...
            setTimeout(function(){
                member.guild.unban(member.id)
                    ...
                    .catch(err => { console.log(err); mine.log("Ban error:\n" + err.message); });
            }, guildSettings[member.guild.id].leavebandays * 86400 * 1000);
        }
    } catch(err){listenerError(err)}
});
...
client.on("guildMemberAdd", (member) => {
    ...
    } catch(err){ console.log(err); mine.log("Add role error:\n" + err.message); };
});
...
async function loadSettings(guilds){
    ...
    } catch(err){ console.log(err); mine.log("Setup roles error:\n" + err.message); };
}
...
function setIntervalsAll(guilds){
    ...
    setInterval(function(){
        for(let i = 0; i < guilds.length; i++){
            mongo.updateSchema({emoteshistory: JSON.stringify(guildSettings[guilds[i]].emoteshistory)}, "settings", guilds[i]);
        }
    }, 3600 *1000); // 1 hour  
    ...
    (function autoSend(){
        setTimeout(function(){
            ...
            autoSend();
        }, (Math.floor(Math.random() * 9) + 20) * 3600000);       
    }());
}
